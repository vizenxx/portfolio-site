    ...
    <ReactLenis root>
      <div className={`w-full ${isMobile ? '' : 'flex items-center justify-center'}`}>
        ...
        <div ref={containerRef} className={`relative w-full transition-colors duration-500 ease-in-out font-sans ${theme.text} ${theme.selection} ${isMobile ? '' : 'overflow-hidden'}`} style={{ '--muted-color': mutedColor }}>
          ...
          {/* UNIFIED BACKGROUNDS (Spotlight + Ripple) with Edge Blur */}
          <div className="fixed inset-0 z-0 pointer-events-none" style={{ maskImage: 'linear-gradient(to bottom, black 85%, transparent 100%)', WebkitMaskImage: 'linear-gradient(to bottom, black 85%, transparent 100%)' }}>
          ...
          {/* NOISE LAYER (Separate, Unmasked, Visible on Mobile) */}
          <div className={`${isMobile ? 'fixed' : 'absolute'} inset-0 z-1 pointer-events-none`}>
